...
export default function ExpenseRevenueList({ expenses, formatCurrency }: ExpenseRevenueListProps) {
  const [activeTab, setActiveTab] = useState<"expenses" | "revenue" | "consolidated">("expenses");
  ...
  const consolidatedItems = useMemo(() => {
    const groups: Record<string, StandardizedExpense[]> = {};
    expenseItems.forEach(item => {
        const category = item.mapped_category || "Uncategorized";
        if (!groups[category]) {
            groups[category] = [];
        }
    ...
    <div className="mt-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center border border-neutral-200 rounded-lg p-0.5 bg-white shadow-inner">
            ...
            className={`px-3 py-1 text-xs font-bold rounded-md transition-colors ${activeTab === "consolidated" ? "bg-neutral-800 text-white shadow-sm" : "text-neutral-600 hover:bg-neutral-100"}`}
            >
            Consolidated
            </button>
            <button
             ...
             <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-neutral-100">
                  {consolidatedItems.length > 0 ? (
                    consolidatedItems.map((group, idx) => (
