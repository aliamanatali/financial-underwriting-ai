          ...
          <table className="w-full text-center h-full table-fixed">
            <thead>
              <tr className="bg-neutral-50 text-[10px] text-neutral-500 border-b border-neutral-100">
                <th className="p-2 border-r border-neutral-100 bg-white w-1/4"></th>
                {growthRates.map((growth, idx) => (
                  <th key={idx} className="p-2 w-1/4">{formatPercent(growth)}</th>
                ))}
              </tr>
            </thead>
            <tbody className="text-xs font-medium">
              {exitCaps.map((cap, rowIdx) => (
                <tr key={rowIdx} className="h-full">
                  <td className="bg-neutral-50 text-neutral-500 border-r border-neutral-100 p-2 align-middle">{formatPercent(cap)}</td>
                  {values[rowIdx]?.map((irr, colIdx) => {
                    const isCurrentCell = rowIdx === 1 && colIdx === 1; // Highlight current parameters
                    return (
                      <td
                        key={colIdx}
                        className={`p-2 align-middle ${getCellColor(irr)} ${isCurrentCell ? 'border border-neutral-900' : ''}`}
                      >
                        {formatPercent(irr)}
                      </td>
                    );
